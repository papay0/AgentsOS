...
import { DirectTTYDInjector } from '@/lib/ttyd-direct-injector';
...
interface CommandPaletteDirectProps {
  terminalUrl: string;
  className?: string;
}
...
export function CommandPaletteDirect({ terminalUrl, className = '' }: CommandPaletteDirectProps) {
  const [injector, setInjector] = useState<DirectTTYDInjector | null>(null);
  ...
  // Initialize injector when terminal URL is available
  useEffect(() => {
    if (terminalUrl && !initRef.current) {
      initRef.current = true;
      console.log('🔗 Initializing Direct TTYDInjector...');
      
      setIsLoading(true);
      
      const initializeInjector = async () => {
        try {
          const directInjector = new DirectTTYDInjector(terminalUrl);
          setInjector(directInjector);
          
          const success = await directInjector.initialize();
          setIsReady(success);
          setLastResult(success ? 'Direct WebSocket connection ready ✅' : 'WebSocket connection failed ❌');
          
        } catch (error) {
          console.error('❌ Direct injector failed:', error);
  ...
  const handleQuickCommand = async (cmd: QuickCommand) => {
    if (!injector || !isReady) {
      setLastResult('❌ Connection not ready');
      return;
    }
  ...
  const handleCustomCommand = async () => {
    if (!injector || !isReady || !customCommand.trim()) {
      setLastResult('❌ Connection not ready or empty command');
      return;
    }
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            Terminal Control (Direct)
          </CardTitle>
          <Badge 
            variant={isReady ? 'default' : 'secondary'}
            className={isReady ? 'bg-green-500' : 'bg-gray-500'}
          >
            {isLoading ? 'Connecting...' : isReady ? 'Connected' : 'Disconnected'}
          </Badge>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {!isReady && !isLoading && (
          <div className="text-center text-gray-500 py-4">
            ...
            <p className="text-sm">Connect to terminal to enable controls</p>
            <p className="text-xs text-gray-400 mt-1">URL: {terminalUrl}</p>
          </div>
        )}
      </CardContent>
    </Card>
